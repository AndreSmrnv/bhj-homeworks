class Autocomplete {
  ...
  getMatches(text) {
    ...
    */
    ...
    for (const elementInputOptions of this.input.options) {
      if (elementInputOptions.text.toLowerCase().includes(text.toLowerCase())) {
        const newMatches = {};
        newMatches.text = elementInputOptions.text;
        newMatches.value = elementInputOptions.value;
        arrayMatches.push(newMatches);
      }
    };
